...
function problemD() {
  ...
  promisifiedReadFile("poem-one/wrong-file-name.txt")
    .then(
      (stanza) => blue(stanza), //handleSucces
      (err) => magenta(new Error(err)) //handleError
    )
    // si el error se filtra del then lo recibe el catch
...
function problemE() {
  ...
  promisifiedReadFile("poem-one/stanza-03.txt")
    .then((stanza) => blue(stanza))
    .then(() => promisifiedReadFile("poem-one/wrong-dile-name.txt"))
    .then((stanza) => {
      blue(stanza);
    })
    .catch((err) => magenta(new Error(err)));
...
function problemF() {
  ...
  promisifiedReadFile("poem-one/stanza-03.txt")
    ...
    .catch((err) => magenta(new Error(err)))
    .done()
    // se ejecuta siempre, devuelve una promesa
